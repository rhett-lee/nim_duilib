...
std::wstring StringConvert::UTF8ToUTF16(const UTF8Char* utf8, size_t length)
{
...
std::wstring StringConvert::UTF32ToUTF16(const UTF32Char* utf32, size_t length)
{
    std::vector<UTF16Char> data;
    data.resize(8192);
    UTF16Char* output = &data[0];
...
std::wstring StringConvert::UTF8ToUTF16(const std::string& utf8)
{
    return UTF8ToUTF16(utf8.c_str(), utf8.length());
}

std::string StringConvert::UTF16ToUTF8(const std::wstring& utf16)
{
...
{
    return StringConvert::UTF16ToUTF8(str);
}
...
{
#ifdef DUILIB_UNICODE
    return StringConvert::UTF8ToUTF16(utf8);
#else
    return utf8;
#endif
}

const std::wstring& StringConvert::TToUTF16(const std::wstring& str)
{
    return str;
}

std::wstring StringConvert::TToUTF16(const std::string& str)
{
    return StringConvert::UTF8ToUTF16(str);
}

#ifdef DUILIB_UNICODE
const DString& StringConvert::UTF16ToT(const std::wstring& utf16)
{
    return utf16;
}
#else
DString StringConvert::UTF16ToT(const std::wstring& utf16)
{
    return StringConvert::UTF16ToUTF8(utf16);
}
#endif
...
std::basic_string<UTF32Char> StringConvert::UTF16ToUTF32(const std::wstring& utf16)
{
    ASSERT(sizeof(UTF16Char) == sizeof(std::wstring::value_type));
    return UTF16ToUTF32((const UTF16Char*)utf16.c_str(), utf16.length());
}

std::wstring StringConvert::UTF32ToUTF16(const std::basic_string<UTF32Char>& utf32)
{
