...
bool WkeMemoryDC::Init(HDC src_dc, int width, int height, const LPVOID pBits/*=NULL*/)
{
	DeleteDC();
...
bool WkeMemoryDC::CreateMemoryDC(HDC src_dc, int width, int height, void** pBits)
{
...
int WkeMemoryDC::GetWidth()
{
	return bitmap_size_.cx;
}

int WkeMemoryDC::GetHeight()
{
	return bitmap_size_.cy;
}

HBITMAP WkeMemoryDC::GetBitmap()
{
	ASSERT(bitmap_ != NULL && valid_);
	return bitmap_;
}

HDC WkeMemoryDC::GetDC()
{
	ASSERT(mem_dc_ != NULL);
	return mem_dc_;
}

bool WkeMemoryDC::IsValid()
{
