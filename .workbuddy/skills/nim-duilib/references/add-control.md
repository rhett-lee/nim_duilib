...
- `class` 属性必须写在所有属性最前面
- `FindControl` 返回的是 `Control*`，务必 `dynamic_cast` 到具体类型并判空
- 引用不存在的 Class 时控件会没有任何样式且**不报错**，写之前先在
  `bin/resources/themes/default/global.xml` 里核对 `<Class name=...>` 是否存在
- 完整的控件属性表见 `api-reference.md` 第三节
