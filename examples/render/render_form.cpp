...
void RenderForm::TestPropertyGrid()
{
	...
	auto p1 = pPropertyGrid->AddProperty(pGroup, L"属性3-0(Spin)", L"3", L"属性3的描述信息");
	p1->SetEnableSpin(true, -10, 10);

	auto p2 = pPropertyGrid->AddProperty(pGroup, L"属性3-1(只读)", L"值3-1", L"属性3的描述信息");
	p2->SetReadOnly(true);

	auto p3 = pPropertyGrid->AddProperty(pGroup, L"属性3-2(密码)", L"值3-2", L"属性3的描述信息");
	p3->SetPassword(true);
}