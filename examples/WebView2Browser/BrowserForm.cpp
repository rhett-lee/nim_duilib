...
BrowserBox* BrowserForm::CreateBox(const std::string &browser_id, DString url)
{
    ...
    GlobalManager::Instance().FillBoxWithCache(tab_item, ui::FilePath(_T("webview2_browser/tab_item.xml")));
    ...
    GlobalManager::Instance().FillBoxWithCache(browser_box, ui::FilePath(_T("webview2_browser/browser_box.xml")), nullptr);
...
bool BrowserForm::AttachBox(BrowserBox* browser_box)
{
    ...
    //重新下载网站图标
    ui::WebView2Control* pWebView2Control = browser_box->GetWebView2Control();
