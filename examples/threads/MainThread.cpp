...
void MainThread::PrintLog(const DString& log)
{
    ...
    DString logMsg = ui::StringUtil::Printf(_T("[%s:%s][%s:%s, %s:%s, %s:%d]: %s"),
        ui::GlobalManager::Instance().Lang().GetStringViaID(_T("STRID_THREADS_EXECUTE_LOG_02_1")).c_str(),
                                            ThreadIdToString(std::this_thread::get_id()).c_str(),
        ui::GlobalManager::Instance().Lang().GetStringViaID(_T("STRID_THREADS_EXECUTE_LOG_02_2")).c_str(),
                                            ThreadIdToString(GetThreadId()).c_str(),
        ui::GlobalManager::Instance().Lang().GetStringViaID(_T("STRID_THREADS_EXECUTE_LOG_02_3")).c_str(),
                                            GetThreadName().c_str(),
        ui::GlobalManager::Instance().Lang().GetStringViaID(_T("STRID_THREADS_EXECUTE_LOG_02_4")).c_str(),
                                            GetThreadIdentifier(),
                                            log.c_str());
    if (m_pMainForm != nullptr) {        
