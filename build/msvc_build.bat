...
SET DUILIB_CMAKE=cmake -G"Visual Studio 17 2022"
SET DUILIB_MAKE=cmake --build
SET DUILIB_BUILD_TYPE=Release
SET DUILIB_BUILD_PARAM=--config %DUILIB_BUILD_TYPE%

@REM # Skia库的子目录，固定使用LLVM编译的，如果想使用默认规则，可注释掉这行
SET DUILIB_SKIA_LIB_SUBPATH=llvm.x64.release
...
@REM # 编译libcef的cef_dll_wrapper
SET DUILIB_THIRD_PARTY_CEF=libcef_win
%DUILIB_CMAKE% -S "%DUILIB_SRC_ROOT_DIR%\duilib\third_party\%DUILIB_THIRD_PARTY_CEF%" -B "%DUILIB_BUILD_DIR%\%DUILIB_THIRD_PARTY_CEF%" -DCMAKE_BUILD_TYPE=%DUILIB_BUILD_TYPE% -DCMAKE_ARCHIVE_OUTPUT_DIRECTORY=%DUILIB_LIB_DIR%
%DUILIB_MAKE% "%DUILIB_BUILD_DIR%\%DUILIB_THIRD_PARTY_CEF%" %DUILIB_BUILD_PARAM%
...
SET DUILIB_PROGRAMS=basic,controls,ColorPicker,DpiAware,layouts,ListBox,ListCtrl,MoveControl,MultiLang,render,RichEdit,VirtualListBox,threads,TreeView,cef,CefBrowser
