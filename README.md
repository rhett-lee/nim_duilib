...
（0）准备工作（Ubuntu/Debian/中科方德）：需要通过`sudo apt install libxext-dev`来安装libxext    
...
（0）准备工作（Fedora）：需要通过`sudo dnf install wayland-devel`来安装libwayland    
（0）准备工作（Fedora）：需要通过`sudo dnf install libxkbcommon-devel`来安装libxkbcommon    
