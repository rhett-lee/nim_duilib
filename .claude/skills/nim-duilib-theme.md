...
> **写 XML 时请一律使用语义色名，不要把下面的色值硬编码进业务布局**，否则深色主题会失效。
>
> 完整色表、命名规范与派生规则见 `docs/ThemeColor.md`。下列仅为浅色主题取值（`font` 色同 `text_`）。
...
> 写 XML 前建议先查一遍，避免引用不存在的 Class（`class` 引用失败时控件会没有任何样式且不报错）。
