...
| :---                  | :---                            |  :---           |   :---  |    :---     | 
| OpenEuler             | 24.03 64位                      | UKUI（X11）     |已完成   |             | 
| OpenEuler             | 24.03 64位                      | DDE （X11）     |已完成   | deepin桌面，不支持半透明窗口，窗口阴影显示为黑色；所以实际只能使用系统标题栏  | 
| 中科方德              | NFSDesktop-5.0-G230-240806 64位 | X11             |已完成   | 程序最大化时会崩溃退出，render运行会退出，与X11有关<br>无法切换到中文输入法，只能输入英文 | 
| 开放麒麟（OpenKylin） | Desktop-V2.0 64位               |Wayland          |已完成   | 程序实际使用的是X11<br>无法切换到中文输入法，只能输入英文<br>子窗口焦点有问题，影响部分功能，比如Combo，鼠标拖动控件操作等<br>| 
| 优麒麟（UbuntuKylin） | ubuntukylin-24.10-desktop 64位  | X11             |已完成   |             | 
| UOS                   |desktop-20 1070                  |                 |进行中...|             | 
| Ubuntu                | 24.04.1 64位                    | GNOME（Wayland）|已完成   | 程序实际使用的是X11<br>窗口弹出有黑屏现象<br>如果强制使用wayland可用运行，但效果较差，不支持DPI缩放，高DPI时界面模糊| 
| Debian                | 12.7.0 64位                     | GNOME（Wayland）|已完成   | 程序实际使用的是X11<br>运行效果较好，窗口弹出时无黑屏现象，但运行速度有点卡，gnome-shell占有CPU很多，自身程序占用CPU很少，应是虚拟机中效率较低<br>无法切换到中文输入法，只能输入英文| 
| Fedora                | Fedora Workstation 40 64位      | GNOME（Wayland）|已完成   | 程序实际使用的是X11<br>窗口弹出时有黑屏现象，<br>无法切换到中文输入法，只能输入英文| 
